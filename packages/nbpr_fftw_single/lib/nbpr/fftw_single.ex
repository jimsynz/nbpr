defmodule NBPR.FftwSingle do
  @moduledoc """
  NBPR package for [`fftw`](http://www.fftw.org) in single precision — the
  library for computing Fast Fourier Transforms, built with `float` as the
  floating-point type.

  Ships `libfftw3f.so.3` under this package's priv dir; `NBPR.Application`
  ...
  that split, so precision is chosen by which package you depend on:
    ...
    * `:nbpr_fftw_single` — `libfftw3f.so.3`, symbols `fftwf_*` (this one)
    * `:nbpr_fftw_double` — `libfftw3.so.3`, symbols `fftw_*`

...
it's timestamped whenever the sentence finished arriving. SHM 1 is the PPS
edge, which is where the microseconds come from — it needs gpsd built with
`pps: true`, a `CONFIG_PPS` kernel, and a wired PPS line. Run gpsd with
`nowait: true` (the default) or the samples stop whenever no client is
connected.

...
Buildroot `fftw-double` package — this release wraps **3.3.10**.
...
This is the one to take if you just want "FFTW" and aren't sure: it's
...
Build options can be overridden in your app's `config/target.exs`:
    ...
    config :nbpr_fftw_double, build_opts: [fast: true]

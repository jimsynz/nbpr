...
NBPR (Nerves Binary Package Repository) is a curated Hex repository for distributing Buildroot-built target packages to Nerves firmware projects. Users declare `{:nbpr_jq, "~> 1.0", organization: "nbpr"}` in their app's `mix.exs` and the binary lands in their rootfs at firmware-build time. Packages with daemons additionally generate MuonTrap-supervised GenServer modules the user adds to their supervision tree. Packages bearing out-of-tree kernel modules generate an `Application` that auto-loads them at boot (`insmod`-by-path via `:nbpr_kmod`'s tools, since stock systems don't index out-of-tree modules). Vendored (out-of-tree) packages ship a `buildroot/` external tree and use `br_external_path:` + `br_packages:` (one nbpr package → several Buildroot packages); see `packages/nbpr_hailo8`.
...
- **Kernel-module packages auto-load via Application.** Packages declaring `kernel_modules: [...]` in their metadata get a generated `NBPR.<Name>.Application` whose `start/2` runs `NBPR.Runtime.load_kernel_module!/2` for each at boot (`insmod` of the priv-shipped `.ko` via `:nbpr_kmod`'s tools with in-tree deps modprobed first; plain `modprobe` as fallback), gated on Nerves target detection (`ID=nerves` in `/etc/os-release`; no-op on host). Kernel-module packages must depend on `:nbpr_kmod`. `Application.stop/1` is a no-op — kmods are global; never `rmmod`. This is kmod-only — daemon packages do *not* auto-start.
- **Hardware-specific packages restrict the matrix.** A package's `targets: [:rpi5]` limits the prebuild matrix to those boards (empty = all). Use for device drivers / SoC accelerators that only apply to certain hardware.
- **Vendored packages stage dev libs for consumer NIFs.** `expose_staging: true` installs the artefact's `staging/` (headers, dev `.so`, CMake/pkg-config) into `priv/staging` at fetch time, so a consumer NIF (e.g. `nx_hailo`) can cross-compile against a shipped library.

defmodule NBPR.BrPackage do
  ...
  @use_opts_schema NimbleOptions.new!(
                     ...
                     rootfs_paths: [
                       ...
                       doc:
                         "Subtrees of the package's `target/` output to route to the real rootfs (`/...`) instead of the package's `priv/`. Default `[\"lib/firmware\"]` (the kernel firmware loader only searches `/lib/firmware`). A shared library that a **NIF** links against needs one of two arrangements: route `\"usr/lib\"` here so it sits on the loader's default path (the BEAM's `dlopen` honours only the `LD_LIBRARY_PATH` captured at process start, so a runtime-set path can't reach a lib in `priv` — see `nbpr_hailo8`), or keep it in `priv` and have the consumer `dlopen` it by absolute path with `RTLD_GLOBAL` before loading its NIF — the namespaced option if several installed packages were to ship colliding sonames. Note routed paths overlay a shared rootfs: two packages routing the same file collide silently, last write wins. Binaries spawned via `System.cmd/2` are fine from `priv` either way; their child process inherits the updated env."
                     ],
                     description: [
                       type: :string,
                       required: true,

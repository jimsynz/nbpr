defmodule NBPR.Buildroot.SystemSource do
  ...
  def ensure!(system_app, system_version)
    ...
    cond do
      ...
      File.regular?(Path.join(deps_path, "nerves_defconfig")) ->
        # Deps tree has source-build inputs (nerves_defconfig, etc.) but
        # is missing `Config.in`. Some systems (e.g. nerves_system_rpi3)
        # don't ship one; the file's only role for our purposes is to be
        # `source`d by `nerves_system_br/Config.in`, and the other
        # rpi-family systems ship just a comment block. Auto-stub it so
        # source-build proceeds.
        File.write!(
          Path.join(deps_path, "Config.in"),
          "# Stub generated by NBPR (#{system_app} v#{system_version} did not ship a Config.in).\n"
        )
